...
if (isset($_GET['id_no'])) {
    if (!empty($_POST)) {
        ...
        // Update the record
        $stmt = $pdo->prepare('UPDATE student_id_registration SET id_no = ?, first_name = ?, last_name = ?, email = ?, phone = ?, address = ?, guardian = ?, course = ?, password = ?, created = ? WHERE id_no = ?');
        $stmt->execute([$id_no, $first_name, $last_name, $email, $phone, $address, $guardian, $course, $password, $created, $_GET['id_no']]);
        $msg = 'Updated Successfully!';
    }
    // Get the contact from the student_info table
    $stmt = $pdo->prepare('SELECT * FROM student_id_registration WHERE id_no = ?');
